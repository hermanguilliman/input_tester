window.GamepadTest = (() => {
    ...
    function loop() {
        if (activeIndex !== null) {
            ...
            if (pad) {
                ...
                const ax = pad.axes;
                if (ax.length >= 2) {
                    stickPos(els.stickL, ax[0], ax[1]);
                    els.stickLLabel.innerText = `L: ${ax[0].toFixed(2)}, ${ax[1].toFixed(2)}`;
                }
                if (ax.length >= 4) {
                    stickPos(els.stickR, ax[2], ax[3]);
                    els.stickRLabel.innerText = `R: ${ax[2].toFixed(2)}, ${ax[3].toFixed(2)}`;
                }
            } else {
                const next = pickActive();
                if (next === null) clearActive();
                else setActive(next);
            }
        }
    ...
    function reset() {
        /* геймпад — состояние реального времени, сбрасывать нечего */
    }
    ...
    function init() {
        const map = {
            ...
            stickL: "stickL",
            stickR: "stickR",
            stickLLabel: "stickLLabel",
            stickRLabel: "stickRLabel",
        };
        Object.keys(map).forEach(
            (k) => (els[k] = document.getElementById(map[k])),
        );
        document
            .getElementById("gpVibrateBtn")
            .addEventListener("click", vibrate);

        window.addEventListener("gamepadconnected", (e) => {
            if (activeIndex === null) setActive(e.gamepad.index);
        });
