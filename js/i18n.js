window.I18n = (() => {
    const DICT = {
        ru: {
            appTitle: "Guilliman Input Diagnostics",
            tabTouch: "Сенсор",
            tabMouse: "Мышь",
            tabKeyboard: "Клавиатура",
            tabGamepad: "Геймпад",
            reset: "Сброс",
            dblThreshold: "Порог Double Click:",
            visualization: "Визуализация",
            scroll: "Скролл",
            idle: "Покой",
            clickZoneHint: "Зона тестирования кликов",
            movement: "Движение",
            speed: "Скорость",
            buttonStats: "Статистика кнопок",
            eventLog: "Лог событий",
            clicks: "Клики",
            double: "Дабл",
            hzLabel: "Частота",
            totalPresses: "Всего нажатий",
            uniqueKeys: "Уникальных клавиш",
            rollover: "Одновременно (NKRO)",
            maxRollover: "Максимум одновременно",
            lastKey: "Последняя клавиша",
            gpConnect: "Подключите геймпад",
            gpConnectHint:
                "Нажмите любую кнопку на контроллере, чтобы активировать его",
            vibrate: "Вибрация",
            buttons: "Кнопки",
            sticks: "Стики",
            down: "Вниз",
            up: "Вверх",
            touchHint: "Коснитесь экрана",
            touchStats: "Статистика касаний",
            activeTouches: "Активных",
            maxConcurrent: "Макс. одновременно",
            totalTouches: "Всего касаний",
            lastGesture: "Жест",
            touchCoords: "Координаты",
            haptic: "Тактильная отдача",
            drawMode: "Рисование",
            hapticWarn: "Вибрация заблокирована браузером. Разрешите вибрацию в настройках сайта.",
            tabMic: "Микрофон",
            micStart: "Старт",
            micStop: "Стоп",
            micAutoMute: "Авто-мьют",
            micIdle: "Ожидание",
            micConnected: "Подключен",
            micLevel: "Уровень",
            micCurrent: "Текущий",
            micPeak: "Пик",
            micClips: "Клиппинг",
            micDuration: "Длительность",
            micDevice: "Устройство",
            tabCamera: "Камера",
            camStart: "Старт",
            camStop: "Стоп",
            camGrid: "Сетка",
            camIdle: "Ожидание",
            camConnected: "Подключена",
            camFps: "FPS",
            camResolution: "Разрешение",
            camDevice: "Устройство",
            camHint: "Нажмите «Старт» для запуска камеры",
        },
        en: {
            appTitle: "Guilliman Input Diagnostics",
            tabTouch: "Touch",
            tabMouse: "Mouse",
            tabKeyboard: "Keyboard",
            tabGamepad: "Gamepad",
            reset: "Reset",
            dblThreshold: "Double Click Threshold:",
            visualization: "Visualization",
            scroll: "Scroll",
            idle: "Idle",
            clickZoneHint: "Click testing area",
            movement: "Movement",
            speed: "Speed",
            buttonStats: "Button Stats",
            eventLog: "Event Log",
            clicks: "Clicks",
            double: "Double",
            hzLabel: "Rate",
            totalPresses: "Total presses",
            uniqueKeys: "Unique keys",
            rollover: "Simultaneous (NKRO)",
            maxRollover: "Max simultaneous",
            lastKey: "Last key",
            gpConnect: "Connect a gamepad",
            gpConnectHint: "Press any button on the controller to activate it",
            vibrate: "Vibrate",
            buttons: "Buttons",
            sticks: "Sticks",
            down: "Down",
            up: "Up",
            touchHint: "Touch the screen",
            touchStats: "Touch Stats",
            activeTouches: "Active",
            maxConcurrent: "Max concurrent",
            totalTouches: "Total touches",
            lastGesture: "Gesture",
            touchCoords: "Coordinates",
            haptic: "Haptic Feedback",
            drawMode: "Draw",
            hapticWarn: "Vibration is blocked by the browser. Allow vibration in site settings.",
            tabMic: "Mic",
            micStart: "Start",
            micStop: "Stop",
            micAutoMute: "Auto-mute",
            micIdle: "Idle",
            micConnected: "Connected",
            micLevel: "Level",
            micCurrent: "Current",
            micPeak: "Peak",
            micClips: "Clipping",
            micDuration: "Duration",
            micDevice: "Device",
            tabCamera: "Camera",
            camStart: "Start",
            camStop: "Stop",
            camGrid: "Grid",
            camIdle: "Idle",
            camConnected: "Connected",
            camFps: "FPS",
            camResolution: "Resolution",
            camDevice: "Device",
            camHint: "Press Start to activate camera",
        },
        zh: {
            appTitle: "Guilliman Input Diagnostics",
            tabTouch: "触屏",
            tabMouse: "鼠标",
            tabKeyboard: "键盘",
            tabGamepad: "手柄",
            reset: "重置",
            dblThreshold: "双击阈值：",
            visualization: "可视化",
            scroll: "滚动",
            idle: "空闲",
            clickZoneHint: "点击测试区域",
            movement: "移动",
            speed: "速度",
            buttonStats: "按键统计",
            eventLog: "事件日志",
            clicks: "点击",
            double: "双击",
            hzLabel: "频率",
            totalPresses: "总按键数",
            uniqueKeys: "独立按键",
            rollover: "同时按下 (NKRO)",
            maxRollover: "最大同时按下",
            lastKey: "最后按键",
            gpConnect: "请连接手柄",
            gpConnectHint: "按下手柄上任意按键以激活",
            vibrate: "震动",
            buttons: "按键",
            sticks: "摇杆",
            down: "向下",
            up: "向上",
            touchHint: "触摸屏幕",
            touchStats: "触摸统计",
            activeTouches: "活动",
            maxConcurrent: "最大同时",
            totalTouches: "总触摸数",
            lastGesture: "手势",
            touchCoords: "坐标",
            haptic: "触觉反馈",
            drawMode: "绘画",
            hapticWarn: "浏览器已阻止震动。请在站点设置中允许震动。",
            tabMic: "麦克风",
            micStart: "开始",
            micStop: "停止",
            micAutoMute: "自动静音",
            micIdle: "空闲",
            micConnected: "已连接",
            micLevel: "电平",
            micCurrent: "当前",
            micPeak: "峰值",
            micClips: "削波",
            micDuration: "时长",
            micDevice: "设备",
            tabCamera: "摄像头",
            camStart: "开始",
            camStop: "停止",
            camGrid: "网格",
            camIdle: "空闲",
            camConnected: "已连接",
            camFps: "帧率",
            camResolution: "分辨率",
            camDevice: "设备",
            camHint: "按「开始」启动摄像头",
        },
    };
    const LANG_NAMES = { ru: "ru", en: "en", zh: "zh" };
    let lang = localStorage.getItem("itp-lang") || "ru";
    if (!DICT[lang]) lang = "ru";

    const listeners = [];
    function t(key) {
        return (DICT[lang] && DICT[lang][key]) || DICT.ru[key] || key;
    }
    function apply() {
        document.querySelectorAll("[data-i18n]").forEach((el) => {
            el.innerHTML = t(el.getAttribute("data-i18n"));
        });
        document.documentElement.lang = LANG_NAMES[lang];
        listeners.forEach((fn) => fn());
    }
    function set(l) {
        if (!DICT[l]) return;
        lang = l;
        localStorage.setItem("itp-lang", l);
        apply();
    }
    function onChange(fn) {
        listeners.push(fn);
    }
    function current() {
        return lang;
    }
    return { t, set, apply, onChange, current };
})();
